...
import (
	...
	"github.com/zzzhr1990/aria2-rpc/rpc2"
)
...
func (c *jsonCodec) ReadHeader(req *rpc2.Request, resp *rpc2.Response) error {
	...
	if c.msg.Method != "" {
		...
		if c.clientResponse.Error != nil || c.clientResponse.Result == nil {
			...
			xx1, ok := c.clientResponse.Error.(map[string]interface{})
			if ok {
				xx2, ok := xx1["code"].(float64)
				if ok {
					resp.Error = fmt.Sprintf("%v", xx2)
				}
				xx3, ok := xx1["message"].(string)
				if ok {
					resp.Error = fmt.Sprintf("%v", xx3)
				}
			} else {
